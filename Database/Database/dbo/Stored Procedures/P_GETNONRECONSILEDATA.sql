...
BEGIN 
	...
	SET @@SQLQUERY = @@SQLQUERY +' SELECT Id,RRN RRNNUMBER,AMOUNT,[DATE], TemplateId,'''+ @PRIMARYTEMPLATENAME +''' TemplateName,IsReconsiled  IsReconsile,ReconsileType,ReconsileDesc,'''+CAST(@@COUNT + 1 AS VARCHAR(11)) +'''  TemplateSpilt  ,CAST(''True'' AS BIT)  IsPrimary 
	...
	SET @@SQLQUERY = @@SQLQUERY +' SELECT Id,RRN,AMOUNT,[DATE],'+CAST(@TEMPLATEID AS VARCHAR(10)) +'  TemplateId,'''+ @TEMPALTENAME +''' [TemplateName],IsReconsiled IsReconsile,ReconsileType,ReconsileDesc,'''+CAST(@@COUNT + 2 AS VARCHAR(11)) +''' TemplateSpilt ,CAST(''False'' AS BIT) IsPrimary FROM RECONSILERAWDATA WHERE ISRECONSILED = 0  
	...
	SET @@SQLQUERYFORRECORDCOUNT = 'SELECT  @NonPrimaryRecord = COUNT(*) OVER () FROM RECONSILERAWDATA WHERE ISRECONSILED = 0  AND TEMPLATEID = '+CAST(@TEMPLATEID AS VARCHAR(10)) +' And CAST(DATE AS DATE) BETWEEN ''' + CAST(@FROMDATE AS  VARCHAR(11)) + ''' and ''' + CAST(@TODATE AS  VARCHAR(11)) + ''''      
	IF(@CONDITIONQUERY != '')        
	BEGIN        
		SET @@SQLQUERYFORRECORDCOUNT = @@SQLQUERYFORRECORDCOUNT +' AND ' + @CONDITIONQUERY        
	END   
...
BEGIN  
	...
	SET @RecordCountDIFF = 0      
	SET @@SQLQUERY = @@SQLQUERY +' UNION ALL '        
	SET @@SQLQUERY = @@SQLQUERY +' SELECT Id,RRN,AMOUNT,[DATE], TemplateId,'''+ @PRIMARYTEMPLATENAME +''' TemplateName,IsReconsiled  IsReconsile,ReconsileType,ReconsileDesc,'''+CAST(@@COUNT + 2 AS VARCHAR(11)) +''' TemplateSpilt,CAST(''True'' AS BIT) IsPrimary 
	FROM PRIMARYRECONSILEDATA WHERE TemplateId in ( '+CAST(@TEMPLATEID AS VARCHAR(10)) +', '+CAST(@PRIMARYTEMPLATEID AS VARCHAR(10)) +') and  
	ID not in (select PrimaryId from  RECONSILERAWDATA WHERE ISRECONSILED =1 AND TEMPLATEID = '+CAST(@TEMPLATEID AS VARCHAR(10)) +' 
	...
	SET @@SQLQUERY = @@SQLQUERY +' UNION ALL '        
	SET @@SQLQUERY = @@SQLQUERY +' SELECT Id,RRN,AMOUNT,[DATE],'+CAST(@TEMPLATEID AS VARCHAR(10)) +'   TemplateId,'''+ @TEMPALTENAME +''' TemplateName,IsReconsiled  IsReconsile,ReconsileType,ReconsileDesc,'''+CAST(@@COUNT + 3 AS VARCHAR(11)) +'''  TemplateSpilt,CAST(''False'' AS BIT) IsPrimary 
	FROM RECONSILERAWDATA WHERE ISRECONSILED = 0  AND TEMPLATEID = '+CAST(@TEMPLATEID AS VARCHAR(10)) +' And CAST(DATE AS DATE) BETWEEN ''' + CAST(@FROMDATE AS  VARCHAR(11)) + ''' and ''' + CAST(@TODATE AS  VARCHAR(11)) + ''''      
