...
SELECT RT.TEMPLATE_GROUP_ID AS GROUP_ID,SUM(AMOUNT) AS CONSILED_AMOUNT, NULL AS NON_CONSILED_AMOUNT,NULL AS PRIMARY_AMOUNT FROM PRIMARYRECONSILEDATA RD
JOIN RECONSILE_TEMPLATE RT ON RD.TEMPLATEID =RT.RECONSILE_TEMPLATE_ID
WHERE RD.LASTUPDATE >= CAST(GETDATE() AS DATE)  AND ISRECONSILED = 1 AND TEMPLATEID IN (SELECT TEMPLATEID FROM TEMPLATE_GROUP WHERE IS_ACTIVE = 1)
...
JOIN RECONSILE_TEMPLATE RT ON RRD.TEMPLATEID =RT.RECONSILE_TEMPLATE_ID  
WHERE RRD.LASTUPDATE >= CAST(GETDATE() AS DATE) AND TEMPLATEID IN (SELECT TEMPLATEID FROM TEMPLATE_GROUP WHERE IS_ACTIVE = 1)  
GROUP BY TEMPLATE_GROUP_ID  
)  
