...
DECLARE @DYNAMICTABLE NVARCHAR(MAX),@DYNAMICJOIN NVARCHAR(MAX) ='',@DYNAMICINSERT NVARCHAR(MAX) ='',@DYNAMICINSERTVALUE NVARCHAR(MAX) ='', @COLUMNCOUNT INT,@TEMPLATE_ID INT = @TEMPLATEID,@ISPRIMARY INT

SET @NUMBEROFCOLUMNS = @NUMBEROFCOLUMNS - 3
SELECT @ISPRIMARY = IsPrimary FROM Reconsile_Template WHERE Reconsile_Template_Id = @TEMPLATEID
...
SET @DYNAMICTABLE = @DYNAMICTABLE + ' AND TARGET.ISRECONSILED =0 AND SOURCE.ISRECONSILED =0 '  
EXEC(@DYNAMICTABLE) 
