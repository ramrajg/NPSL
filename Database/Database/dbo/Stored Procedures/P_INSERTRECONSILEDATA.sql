...
SET @DYNAMICTABLE = '' 
SET @DYNAMICTABLE = 'ALTER TABLE  ##TEMPRECONSILEDATERAW ADD TEMPLATEID INT NOT NULL DEFAULT(' + CAST(@TEMPLATE_ID AS VARCHAR(10)) + ')'
EXEC(@DYNAMICTABLE);
...
BEGIN
	SET @DYNAMICTABLE =''
	SET @DYNAMICTABLE ='SELECT RRN,[DATE],AMOUNT,TEMPLATEID,FILENAME'  + @DYNAMICINSERT  
...
SET @DYNAMICTABLE ='' 
SET @DYNAMICTABLE = 'INSERT INTO ##TEMPRECONSILEDATE(RRN,[DATE],AMOUNT,TEMPLATEID'  + @DYNAMICINSERT  
SET @DYNAMICTABLE = @DYNAMICTABLE + ') SELECT SOURCE.RRN, CAST(SOURCE.[DATE] AS DATE) [DATE], SOURCE.AMOUNT,SOURCE.TEMPLATEID' + @DYNAMICINSERTVALUE + ' FROM PRIMARYRECONSILEDATA SOURCE JOIN RECONSILERAWDATA TARGET ON'
...
SET @DYNAMICTABLE = @DYNAMICTABLE + ') VALUES (SOURCE.RRN, SOURCE.[DATE], SOURCE.AMOUNT,TEMPLATEID' + @DYNAMICINSERTVALUE  
SET @DYNAMICTABLE = @DYNAMICTABLE + ');'  
EXEC(@DYNAMICTABLE) 
