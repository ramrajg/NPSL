...
declare @sumAmountNonPrimary numeric(18,2) = (select sum(Amount) from RECONSILERAWDATA where id in (select Id from @pSelectedResult where [Type] ='NP'))
...
begin
	...
	UPDATE RECONSILERAWDATA SET ISRECONSILED = 1,RECONSILETYPE = 'M' WHERE ID IN (SELECT ID FROM @pSelectedResult WHERE [TYPE] ='NP')
end
...
begin
	...
	SELECT RRN,GETDATE(),@DIFFAMOUNT,RECOCOL4,RECOCOL5,RECOCOL6,RECOCOL7,RECOCOL8,RECOCOL9,RECOCOL10,RECOCOL11,RECOCOL12,RECOCOL13,RECOCOL14,RECOCOL15,RECOCOL16,FILENAME,TEMPLATEID,0,0,'T',RECONSILEDESC FROM RECONSILERAWDATA WHERE ID  IN (SELECT TOP 1 ID FROM @pSelectedResult WHERE [TYPE] ='NP')
	UPDATE PRIMARYRECONSILEDATA SET ISRECONSILED = 1,RECONSILETYPE = 'M' WHERE ID IN (SELECT ID FROM @pSelectedResult WHERE [TYPE] ='P')
	UPDATE RECONSILERAWDATA SET ISRECONSILED = 1,RECONSILETYPE = 'M' WHERE ID IN (SELECT ID FROM @pSelectedResult WHERE [TYPE] ='NP')
end
else if (@sumAmountPrimary=@sumAmountNonPrimary)
...
UPDATE RECONSILERAWDATA SET ISRECONSILED = 1,RECONSILETYPE = 'M' WHERE ID IN (SELECT ID FROM @pSelectedResult WHERE [TYPE] ='NP')
 ...
 EXEC  P_GETNONRECONSILEDATA @GROUPTEMPLATEID,@FROMDATE,@TODATE
END