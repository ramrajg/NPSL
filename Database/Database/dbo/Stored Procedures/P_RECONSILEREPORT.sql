...
IF @@COUNT = 0 
BEGIN
	SET @@SQLQUERY = 'INSERT INTO ##TEMPRECONSILEREPORT SELECT ''RECONSILED FOR TEMPLATE : ' + @TEMPALTENAME + ''' AS [DETAIL RRN],ISNULL(SUM(AMOUNT),0) AS [AMOUNT],'''' AS [DATE] FROM PRIMARYRECONSILEDATA WHERE ISRECONSILED =1 AND TEMPLATEID = '+CAST(@TEMPLATEID AS VARCHAR(10))
	...
	SET @@SQLQUERY = @@SQLQUERY +' UNION ALL '
	SET @@SQLQUERY = @@SQLQUERY +' SELECT ''NON RECONSILED FOR TEMPLATE : ' + @TEMPALTENAME + ''' AS [DETAIL RRN],ISNULL(SUM(AMOUNT),0) AS [AMOUNT],'''' AS [DATE] FROM PRIMARYRECONSILEDATA WHERE ISRECONSILED = 0 AND TEMPLATEID  IN (SELECT RECONSILE_TEMPLATE_ID FROM RECONSILE_TEMPLATE WHERE TEMPLATE_GROUP_ID = ' + CAST(@GROUPTEMPLATEID AS VARCHAR(10)) +')'
	IF(@CONDITIONQUERY != '')
	BEGIN
...
BEGIN
	SET @@SQLQUERY = @@SQLQUERY +' UNION ALL '
	SET @@SQLQUERY = @@SQLQUERY +' SELECT ''RECONSILED FOR TEMPLATE : ' + @TEMPALTENAME + ''' AS [DETAIL RRN],SUM(AMOUNT) AS [AMOUNT],'''' AS [DATE] FROM PRIMARYRECONSILEDATA WHERE ISRECONSILED =1 AND  TEMPLATEID = '+CAST(@TEMPLATEID AS VARCHAR(10))
	SET @@SQLQUERY = @@SQLQUERY +' UNION ALL '
	SET @@SQLQUERY = @@SQLQUERY +' SELECT RRN,AMOUNT,[DATE] FROM PRIMARYRECONSILEDATA WHERE ISRECONSILED =1 AND TEMPLATEID = '+CAST(@TEMPLATEID AS VARCHAR(10))
	SET @@SQLQUERY = @@SQLQUERY +' UNION ALL '
	SET @@SQLQUERY = @@SQLQUERY +' SELECT ''NON RECONSILED FOR TEMPLATE : ' + @TEMPALTENAME + ''' AS [DETAIL RRN],SUM(AMOUNT) AS [AMOUNT],'''' AS [DATE] FROM PRIMARYRECONSILEDATA WHERE ISRECONSILED <> 0 AND TEMPLATEID  IN (SELECT RECONSILE_TEMPLATE_ID FROM RECONSILE_TEMPLATE WHERE TEMPLATE_GROUP_ID = ' + CAST(@GROUPTEMPLATEID AS VARCHAR(10)) +')'
	IF(@CONDITIONQUERY != '')
	BEGIN
		SET @@SQLQUERY = @@SQLQUERY +' AND ' + @CONDITIONQUERY
	END
