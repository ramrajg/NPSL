    ...
    public class ReconsileReportController : Controller
    {
        public IActionResult Index()
        {
           var formattedDate = DateTime.Today.ToString("dd MMM yyyy");
            var ReconsileReportResult = CustomUtility.GetSingleRecord<ReconsileReportData>(string.Format("api/GetReconsileReportData?groupTemplateId=0&fromDate=" + formattedDate + "&toDate=" + formattedDate + ""));
        ...
        public PartialViewResult RefreshSearchResult(int groupId,DateTime FromDate,DateTime ToDate)
        {
            ...
            var ReconsileReportResult = CustomUtility.GetSingleRecord<ReconsileReportData>(string.Format("api/GetReconsileReportData?groupTemplateId=" + groupId + "&fromDate=" + From_Date + "&toDate=" + To_Date + ""));
            ViewModel mymodel = new ViewModel();
            mymodel.ReportData = ReconsileReportResult;
